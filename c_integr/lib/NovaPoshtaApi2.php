...
//			throw new \Exception('Region is required filed for recipient');
	...
	function newInternetDocument($sender, $recipient, $params) {
		...
		} else if($sender['WerehouseRef']) {
			$sender['SenderAddress'] = $sender['WerehouseRef'];
		}
		if ( ! $sender['Sender']) {
			...
			//print_r($senderCounterpartyExisting);exit;
			...
			if ($senderCounterpartyExisting['data'][0]['Ref']) {
				...
				$sender['CounterpartyType'] = 'PrivatePerson';
				$recipientCounterparty = $this->model('Counterparty')->save($sender);
				print_r($recipientCounterparty);exit;
	    			$sender['Sender'] = $recipientCounterparty['data'][0]['Ref'];
	    			$contactSender = $this->getCounterpartyContactPersons($sender['Sender']);
	    			$sender['ContactSender'] = $contactSender['data'][0]['Ref'];
	    			$sender['SendersPhone'] = $sender['Phone'] ? $sender['Phone'] : $contactSender['data'][0]['Phones'];
			}*/
		...
		if ( ! $recipient['RecipientAddress']&&!$recipient['RecipientAddressRef']) {
		...
		} else if($recipient['RecipientAddressRef']) {
			$recipient['RecipientAddress'] = $recipient['RecipientAddressRef'];
			unset($recipient['RecipientAddressRef']);
		}
	 ...
	 * @return mixed
	 */
	protected function printGetLink($method, $documentRefs, $type = 'pdf_link') {
	 ...
	 * @return mixed
	 */
	function printDocument($documentRefs, $type = 'pdf_link') {
