...
class ControllerInformationInformation extends Controller {
	public function index() {
		...
		if ($information_info) {
            ...
            preg_match_all($search, $data['description'], $matches);
                ...
                foreach ($matches[1] as $key => $image) {
                    if (is_file(DIR_IMAGE . $image)) {
                        ...
                        if (!empty($image_size[0]) && !empty($image_size[1]) && !empty($image_size[3])) {
                            $thumb = $this->model_tool_image->resize($image, $image_size[0], $image_size[1]);

                            $replace = '<img src="' . $thumb . '" ' . $image_size[3] .' class="img-responsive" loading="lazy"/>';

                            $search = '/\[image=\"' . preg_replace('/\//', '\/', $image). '\"]/';

                            $data['description'] = preg_replace($search, $replace, $data['description']);
                        }
                    }
                }
            }
