...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['link'] = $this->url->link('account/order/info','order_id=' . $order_info['order_id']);
		if ($download_status) {
        ...
        //#++
        $comment_to_admin = strip_tags($order_info['comment']);
        ...
        $data['comment'] = $comment_to_admin;
        $mail->setHtml($this->load->view('mail/order_add_admin', $data));
        $mail->send();
