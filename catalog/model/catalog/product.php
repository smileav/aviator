...
class ModelCatalogProduct extends Model {
    ...
    public function getProductSpecials($data = array()) {
        $config_language_id = (int)$this->config->get('config_language_id');
        $NOW = date('Y-m-d H:i') . ':00';
        // $sql = "SELECT DISTINCT ps.product_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ps.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND `p`.`archive` = '0' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) GROUP BY ps.product_id";

        $sql = "SELECT DISTINCT ps.product_id FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.status = '1' AND `p`.`archive` = '0' AND p.date_available <= NOW() AND pd.language_id = '" . $config_language_id . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $NOW . "')) GROUP BY ps.product_id";
	...
	public function getLatestProducts($limit) {
		...
		if (!$product_data) {
            ...
            $query = $this->db->query("SELECT DISTINCT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p2s.product_id = p2c.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p2c.category_id <> '206' ORDER BY p.date_added DESC LIMIT " . (int)$limit);
    ...
    public function getProductThumbOptions($product_id) {
    ...
    public function getProductColor($product_id) {
        $query = $this->db->query("SELECT `p`.`product_id`, `p`.`image`, `pd`.`name` FROM `" . DB_PREFIX . "product_color` `pc` LEFT JOIN `" . DB_PREFIX . "product` `p` ON (`pc`.`color_id` = `p`.`product_id`) LEFT JOIN `" . DB_PREFIX . "product_description` `pd` ON (`p`.`product_id` = `pd`.`product_id`) WHERE `pc`.`product_id` = '" . (int)$product_id . "' AND `p`.`status` = '1' AND `p`.`archive` = '0' AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "' ORDER BY `pc`.`sort_order` ASC");
    ...
    public function getProductKit($product_id) {
        ...
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_kit` WHERE `product_id` = '" . (int)$product_id . "' ORDER BY `sort_order` LIMIT 1");

        foreach ($query->rows as $result) {
            $product_data[$result['kit_id']] = $result['product_id'];
        }

        if (!$product_data) {
            $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_kit` WHERE `kit_id` = '" . (int)$product_id . "' ORDER BY `sort_order` LIMIT 1");
