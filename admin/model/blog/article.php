...
class ModelBlogArticle extends Model {
	public function addArticle($data) {
        ...
        if (isset($data['image_short'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "article SET image_short = '" . $this->db->escape($data['image_short']) . "' WHERE article_id = '" . (int)$article_id . "'");
        }

		foreach ($data['article_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "article_description SET article_id = '" . (int)$article_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', link = '" . $this->db->escape($value['link']) . "', link_text = '" . $this->db->escape($value['link_text']) . "'");
		}
