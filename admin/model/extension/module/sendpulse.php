...
class ModelExtensionModuleSendpulse extends Model
{
    ...
    public function export($id, $secret, $id_book)
    {
        ...
        if ($id != '' && $secret != '' && $id_book != '') {
            $query = $this->db->query("SELECT `email`, `firstname`, `lastname`, `telephone` FROM `" . DB_PREFIX . "customer`");
    ...
    public function exportNewCustomer($data)
    {
        $err = 0;
        $this->load->language('extension/module/sendpulse');
        $isSubscribed = !empty($data['newsletter']) && $data['newsletter'] == 1;
        if ($this->config->get('module_sendpulse_status') == 1 && $this->config->get('module_sendpulse_auto_add') == 1 && $this->config->get('module_sendpulse_book_default') > 0 && $isSubscribed) {
            $emails = array();
            $emails[] = array(
                'email' => $data['email'],
                'variables' => array(
