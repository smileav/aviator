...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', noindex = '" . (int)$data['noindex'] . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', date_modified = NOW(), date_added = NOW()");

		$category_id = $this->db->getLastId();

        if (!empty($data['rz_category'])) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "category_to_rozetka SET category_id = '" . (int)$category_id . "', rz_id = '" . (int)$data['rz_category'] . "'");
        }
	...
	public function editCategory($category_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', noindex = '" . (int)$data['noindex'] . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");

        $this->db->query("DELETE FROM " . DB_PREFIX . "category_to_rozetka WHERE category_id = '" . (int)$category_id . "'");
    ...
    public function getRozetkaCategory($category_id) {
        $query = $this->db->query("SELECT cr.rz_name, cr.rz_id FROM " . DB_PREFIX . "category_to_rozetka c2r LEFT JOIN " . DB_PREFIX . "rozetka cr ON (c2r.rz_id = cr.rz_id) WHERE c2r.category_id = '" . (int)$category_id . "'");

        return $query->row;
    }

    public function getRozetkaCategories($data = array()) {
        $sql = "SELECT rz_id, rz_name FROM " . DB_PREFIX . "rozetka";

        if (!empty($data['filter_name'])) {
            $sql .= " WHERE rz_name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
        }
