...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
        ...
        //# <!-- Архивный товар
        if (!$data['archive'] && $data['stock_status_id'] == 12) {
            $data['stock_status_id'] = 5;
        } elseif ($data['archive']) {
            $data['stock_status_id'] = 12;
        }
        //# Архивный товар -->

		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', `archive` = '" . (int)$data['archive'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', noindex = '" . (int)$data['noindex'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW()");

		$product_id = $this->db->getLastId();

        if (isset($data['cyl_id']) && isset($data['cyl_status']) && !empty($data['cyl_data'])) {
            $this->load->model('extension/complete_your_look');

            $this->model_extension_complete_your_look->setLook($product_id, $data);
        }
        ...
        if (isset($data['product_color'])) {
            foreach ($data['product_color'] as $product_color) {
                $this->db->query("DELETE FROM `" . DB_PREFIX . "product_color` WHERE product_id = '" . (int)$product_id . "' AND `color_id` = '" . (int)$product_color['product_id'] . "'");
                $this->db->query("INSERT INTO `" . DB_PREFIX . "product_color` SET `product_id` = '" . (int)$product_id . "', `color_id` = '" . (int)$product_color['product_id'] . "', `sort_order` = '" . (int)$product_color['sort_order'] . "'");
                $this->db->query("DELETE FROM `" . DB_PREFIX . "product_color` WHERE `product_id` = '" . (int)$product_color['product_id'] . "' AND `color_id` = '" . (int)$product_id . "'");
                $this->db->query("INSERT INTO `" . DB_PREFIX . "product_color` SET `product_id` = '" . (int)$product_color['product_id'] . "', `color_id` = '" . (int)$product_id . "', `sort_order` = '" . (int)$data['product_color_sort_order'] . "'");
            }
        }

        if (isset($data['product_kit'])) {
            foreach ($data['product_kit'] as $product_kit) {
                $this->db->query("DELETE FROM `" . DB_PREFIX . "product_kit` WHERE product_id = '" . (int)$product_id . "' AND `kit_id` = '" . (int)$product_kit['product_id'] . "'");
                $this->db->query("INSERT INTO `" . DB_PREFIX . "product_kit` SET `product_id` = '" . (int)$product_id . "', `kit_id` = '" . (int)$product_kit['product_id'] . "', `sort_order` = '" . (int)$product_kit['sort_order'] . "'");
                $this->db->query("DELETE FROM `" . DB_PREFIX . "product_kit` WHERE `product_id` = '" . (int)$product_kit['product_id'] . "' AND `kit_id` = '" . (int)$product_id . "'");
                $this->db->query("INSERT INTO `" . DB_PREFIX . "product_kit` SET `product_id` = '" . (int)$product_kit['product_id'] . "', `kit_id` = '" . (int)$product_id . "', `sort_order` = '" . (int)$data['product_kit_sort_order'] . "'");
            }
        }
	...
	public function getProducts($data = array()) {;
        ...
        if (!empty($data['filter_color'])) {
            $sql .= " AND (`pd`.`name` LIKE '%" . $this->db->escape($data['filter_color']) . "%' OR `p`.`model` LIKE '" . $this->db->escape($data['filter_color']) . "%')";
        }

        if (isset($data['filter_kit'])) {
            $sql .= " AND `p`.`status` = '1'";

            if (!empty($data['filter_kit'])) {
                $sql .= " AND (`pd`.`name` LIKE '%" . $this->db->escape($data['filter_kit']) . "%' OR `p`.`model` LIKE '" . $this->db->escape($data['filter_kit']) . "%')";
            }
        }
        ...
        if (isset($data['filter_cyl']) && $data['filter_cyl']) {
            $sql .= " AND p.cyl_id = '0'";
        }
    ...
    public function getProductColorSortOrder($product_id) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_color` WHERE `color_id` = '" . (int)$product_id . "' LIMIT 1");
    ...
    public function getProductColor($product_id) {
        ...
        foreach ($query->rows as $result) {
            $product_color_data[$result['color_id']] = $result['sort_order'];
        }

        return $product_color_data;
    }

    public function getProductKit($product_id) {
        $product_kit_data = array();

        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_kit` WHERE `product_id` = '" . (int)$product_id . "'");


        foreach ($query->rows as $result) {
            $product_kit_data[$result['kit_id']] = [
                'product_id' => $result['product_id'],
                'sort_order' => $result['sort_order'],
            ];
        }

        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_kit` WHERE `kit_id` = '" . (int)$product_id . "'");
