...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['rozetka_category'])) {
            $data['rozetka_category'] = $this->request->post['rozetka_category'];
        } elseif (isset($this->request->get['category_id'])) {
            $data['rozetka_category'] = $this->model_catalog_category->getRozetkaCategory($this->request->get['category_id']);
        } else {
            $data['rozetka_category'] = array();
        }
        ...
        if (isset($this->request->post['banner'])) {
            $data['image'] = $this->request->post['banner'];
        } elseif (!empty($category_info)) {
    ...
    public function rzList() {
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $results = $this->model_catalog_category->getRozetkaCategories($filter_data);

            foreach ($results as $result) {
                $json[] = array(
                    'rz_id' => $result['rz_id'],
                    'rz_name'        => strip_tags(html_entity_decode($result['rz_name'], ENT_QUOTES, 'UTF-8'))
                );
            }
