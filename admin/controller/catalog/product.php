...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            //# Product Meta
            if ($this->config->get('product_meta_status')) {
                $this->load->model('catalog/category');

                $main_category_data = $this->model_catalog_category->getCategoryDescriptions($this->request->post['main_category_id']);

                $product_meta = $this->config->get('product_meta_data');

                $this->load->model('localisation/language');

                foreach ($this->model_localisation_language->getLanguages() as $language) {
                    if (empty($this->request->post['product_description'][$language['language_id']]['meta_h1'])) {
                        $this->request->post['product_description'][$language['language_id']]['meta_h1'] = preg_replace(
                            [
                                '/\{name\}/',
                                '/\{manufacturer\}/',
                                '/\{category\}/'],
                            [
                                ...
                                $this->request->post['manufacturer'],
                                $main_category_data[$language['language_id']]['name'],
                            ],
                            $product_meta[$language['language_id']]['meta_h1']
                        );
                    }

                    if (empty($this->request->post['product_description'][$language['language_id']]['meta_title'])) {
                        $this->request->post['product_description'][$language['language_id']]['meta_title'] = preg_replace(
                            [
                                '/\{name\}/',
                                '/\{manufacturer\}/',
                                '/\{category\}/'],
                            ...
                            $product_meta[$language['language_id']]['meta_title']
                        );
                    }

                    if (empty($this->request->post['product_description'][$language['language_id']]['meta_description'])) {
                        $this->request->post['product_description'][$language['language_id']]['meta_description'] = preg_replace(
                            [
                                '/\{name\}/',
                                '/\{manufacturer\}/',
                                '/\{category\}/'],
                            ...
                            $product_meta[$language['language_id']]['meta_description']
                        );
                    }
                }
            }
	...
	protected function getForm() {
        ...
        //# <!-- Архивный товар
        if (isset($this->request->post['archive'])) {
            $data['archive'] = $this->request->post['archive'];
        } elseif (!empty($product_info)) {
        ...
        // Color
        if (isset($this->request->post['product_color_sort_order'])) {
            $data['product_color_sort_order'] = $this->request->post['product_color_sort_order'];
        } elseif (isset($this->request->get['product_id'])) {
            $data['product_color_sort_order'] = $this->model_catalog_product->getProductColorSortOrder($this->request->get['product_id']);
        } else {
            $data['product_color_sort_order'] = 0;
        }
        ...
        foreach ($products as $color_id => $sort_order) {
            $color_info = $this->model_catalog_product->getProduct($color_id);

            if ($color_info) {
                if (is_file(DIR_IMAGE . $color_info['image'])) {
                    $image = $this->model_tool_image->resize($color_info['image'], 40, 40);
                } else {
                    $image = $this->model_tool_image->resize('no_image.png', 40, 40);
                }
        ...
        // Kit
        if (isset($this->request->post['product_kit'])) {
            $products = $this->request->post['product_kit'];
        } elseif (isset($this->request->get['product_id'])) {
            $products = $this->model_catalog_product->getProductKit($this->request->get['product_id']);
        } else {
            $products = [];
        }
        ...
        foreach ($products as $kit_id => $result) {
            if ($kit_id == $this->request->get['product_id']) {
                $kit_info = $this->model_catalog_product->getProduct($this->request->get['product_id']);
            } else {
                $kit_info = $this->model_catalog_product->getProduct($kit_id);
            }

            if ($kit_info) {
                if (is_file(DIR_IMAGE . $kit_info['image'])) {
                    $image = $this->model_tool_image->resize($kit_info['image'], 40, 40);
                } else {
                    $image = $this->model_tool_image->resize('no_image.png', 40, 40);
                }
                ...
                $data['product_kits'][] = [
                    'kit_id'        => $kit_id,
                    'model'         => $kit_info['model'],
                    'thumb'         => $image,
                    'name'          => $kit_info['name'],
                    'sort_order'    => $result['sort_order'],
                    'edit'          => $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $kit_id . $url, true)
                ];
            }
        }
        ...
        // CompleteYourLook
        if (isset($product_info['cyl_id']) && $product_info['cyl_id']) {
            $cyl_id = $product_info['cyl_id'];
        } else {
            $cyl_id = 0;
        }

        $data['cyl'] = $this->load->controller('extension/complete_your_look', [ 'cyl_id' =>  $cyl_id ]);
	...
	protected function validateForm() {
        ...
        if (!empty($this->request->post['product_seo_url'])) {
            ...
            if ($_keyword === false) {
                foreach ($this->request->post['product_description'] as $language_id => $value) {
                    $_keyword = $this->makeKeyword($value['name']);

                    foreach ($this->request->post['product_seo_url'] as $store_id => $language) {
                        foreach ($language as $language_id => $keyword) {
                            $this->request->post['product_seo_url'][$store_id][$language_id] = $_keyword;
                        }
                    }
    ...
    public function makeKeyword($str) {
        ...
        return preg_replace('/[^a-z0-9_\-]/', '', $keyword);
    }
}
