...
class ControllerCatalogManufacturer extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $category_id = $this->toCategory($this->request->get['manufacturer_id'], $this->request->post);

            $this->model_catalog_manufacturer->updateProductCategory($this->request->get['manufacturer_id'], $category_id);
    ...
    public function toCategory($manufacturer_id, $data) {
        $this->load->model('catalog/category');

        $category_id = $this->model_catalog_manufacturer->getCategoryByManufacturerId($manufacturer_id);
        ...
        foreach ($data['manufacturer_description'] as $language_id => $result) {
            $category_data['category_description'][$language_id] = $result;
            $category_data['category_description'][$language_id]['name'] = $name;
        }

        $category_data['path'] = '';
        $category_data['parent_id'] = 0;
        $category_data['category_store'] = [ 0 ];
        $category_data['image'] = $data['image'];

        if (!empty($category_info['banner'])) {
            $category_data['banner'] = $category_info['banner'];
        } else {
        ...
        $category_data['sort_order'] = 1000 + $data['sort_order'];

        if (isset($category_info['status'])) {
            $category_data['status'] = $category_info['status'];
        } else {
        ...
        if (!empty($this->request->post['manufacturer_seo_url'][0])) {
            foreach ($this->request->post['manufacturer_seo_url'][0] as $language_id => $keyword) {
                $keyword = ltrim($keyword, '__MAN__');

                $category_data['category_seo_url'][0][$language_id] = $keyword;

                $this->request->post['manufacturer_seo_url'][0][$language_id] = '__MAN__' . $keyword;
            }
        }

        $category_data['noindex'] = 1;

        if (isset($category_info['product_related'])) {
            $category_data['product_related'] = $category_info['product_related'];
        }

        if (isset($category_info['article_related'])) {
            $category_data['article_related'] = $category_info['article_related'];
        }

        $category_data['category_layout'] = [ 0 ];

        $category_data['manufacturer_id'] = $manufacturer_id;
